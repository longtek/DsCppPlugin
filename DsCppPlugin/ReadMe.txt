========================================================================
    活动模板库 : DsCppPlugin 项目概述
========================================================================

应用程序向导已为您创建了此 DsCppPlugin 项目，作为编写动态链接库 (DLL) 
的起点。
...
DsCppPlugin.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(DsCppPlugin.h)
        GUID 定义			(DsCppPlugin_i.c)
        类型库				(DsCppPlugin.tlb)
        封送处理代码			(DsCppPlugin_p.c 和 
                                           dlldata.c）

DsCppPlugin.h
    此文件包含 DsCppPlugin.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 DsCppPlugin.pch 的预编译头 (PCH) 文件和
...
DsCppPluginps.vcxproj
    此文件是用于生成代理/存根 (stub) DLL 的项目文件（若有必要）。主项目中的 IDL 
    文件必须至少包含一个接口，并且在生成代理/存根 (stub) DLL 之前必须先编译 IDL 
    文件。此进程生成 dlldata.c、DsCppPlugin_i.c 和 
    DsCppPlugin_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

DsCppPluginps.vcxproj.filters
